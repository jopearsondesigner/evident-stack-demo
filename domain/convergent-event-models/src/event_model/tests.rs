use super::ConvergentEventModel;
use crate::converge::opset::InMemoryOpSet;
use event_models::{
    types::{Described, Entity, Named},
    EventModelModifier,
};
use uuid::Uuid;

// TODO: figure out how to encapsulate these tests to apply to any EventModel impl

#[test]
fn successful_creation() {
    let id = Uuid::new_v4();
    let name = "foo";
    let result = ConvergentEventModel::new(id, name, 12, InMemoryOpSet::default());
...
fn renaming() {
    let id = Uuid::new_v4();
    let initial = ConvergentEventModel::new(id, "foo", 12, InMemoryOpSet::default());
...
//     let id = Uuid::new_v4();
//     let initial = ConvergentEventModel::new(id, "foo".to_string());
//     let result = initial
