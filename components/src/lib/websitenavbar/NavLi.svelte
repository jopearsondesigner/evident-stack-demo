<script lang="ts">
  ...
  export {navLiClass as class}
  let navLiClass:string;
  $: navLiClass = classNames(
  ...
  let mobileNavLi='p-4 transition duration-200 ease-in-out w-full';
...
{:else if footer}
<li class={classNames('flex justify-center', navLiClass, mobileNavLi)}>
  <svelte:element
    this={href ? 'a' : 'div'}
    {href}
    {...$$restProps}
...
{:else}
<li class={classNames(navLiClass, mobileNavLi)}>
  <svelte:element
    this={href ? 'a' : 'div'}
    {href}
    {...$$restProps}
