<script lang="ts">
  ...
  <li use:clickOutside={() => !hidden && handleDropdown()} class={liClass}>
	<button on:click={() => handleDropdown()} class={liButtonClass}
	  >{name}
	  <Icon name='arrow-down' class='ml-1 stroke-2' iconColor='fill-current' size={12}><ArrowDown/></Icon>
	  {#if website}
	  <Icon name='nav-arrow-light' class='block dark:hidden z-50 w-6 absolute mt-[37px] transform'><NavArrowLight/></Icon>
	  <Icon name='nav-arrow-dark' class='hidden dark:block z-50 w-6 absolute mt-[37px] transform'><NavArrowDark/></Icon>
	  {/if}
  </button>
