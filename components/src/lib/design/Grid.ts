...
export type PlacementCell = {
  placement: InterfacePlacement | TimelinePlacement | EventPlacement;
  empty?: false;
  audience?: null;
  stream?: null;
};
export type EmptyInterfaceCell = {
  placement: null;
  empty: 'interface';
  audience?: string;
  stream?: null;
};
export type EmptyTimelineCell = {
  placement: null;
  empty: 'timeline';
  audience?: null;
  stream?: null;
...
export const itemAtCursor = (
  ...
  if (row === 0) {
    ...
    return placement
      ? { placement }
      : { empty: 'interface', audience: audience?.id, placement: null };
