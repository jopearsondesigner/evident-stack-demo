...
<div class="mt-16">
  {#if state != 'success'}
    <form on:submit|preventDefault={handleSubmit}>
      ...
      {#if state == 'submitting'}
        <p>emailing {email}...</p>
      {:else}
        <Button size="xl" color="primary" type="submit" class="my-4"
          ><span slot="label">send magic link</span></Button
        >
      {/if}
  ...
  {:else}
    <div>
      <p>
        We've sent your login email to {email}. Please find it in your inbox and click the link
        there to login.
      </p>
    </div>
  {/if}
